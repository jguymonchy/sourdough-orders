...
export async function POST(req: NextRequest) {
  try {
    ...
    // Build the HTML once (used in both emails)
    const html = `
<h2>New order</h2>
...
`.trim();
    ...
    // 1) Admin email (reply goes to the customer if provided)
    console.log('order1 admin email', { to: adminTo, orderId: inserted.id });
    await sendOrderEmail({
      to: adminTo,
      subject: `New Order: ${inserted.customer_name}`,
      html,
      replyTo: data.email?.trim() || undefined,
    });

    // 2) Customer email (only if provided)
    if (data.email?.trim()) {
      const customerHtml = `
...
`.trim();
      ...
      console.log('order1 customer email', {
        to: data.email.trim(),
        replyTo: adminTo,
      });
      ...
      await sendOrderEmail({
        ...
        html: customerHtml,
        replyTo: adminTo, // their replies go to you
      });
    } else {
      console.log('order1 no customer email provided');
    }

    return NextResponse.json({ ok: true });
  } catch (e: any) {
    console.error('order1 error', e?.message || e);
    return new NextResponse('Bad Request', { status: 400 });
  }
}

// Optional: admin list endpoint (kept as before, protected by middleware)
