...
export default function OrderPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Build payload with multiple field aliases so the API can normalize
      const payload = {
        // canonical
      ...
      const raw = await res.text(); // avoid the “unexpected end of JSON” crash
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              {items.length > 0 && (
                <div style={{ border: '1px solid #eee', borderRadius: 10 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 80px',
                                padding: '8px 10px', borderBottom: '1px solid #eee', fontWeight: 600 }}>
                    <div>Item</div>
                  ...
                  {items.map((line, idx) => (
                    <div key={`${line.name}-${idx}`}
                      style={{ display: 'grid', gridTemplateColumns: '1fr 120px 80px',
                      ...
                      <div>
                        <button type="button" onClick={() => removeItem(idx)}
                          style={{ appearance: 'none', border: 'none', background: '#eee', color: '#333',
